...
export default function App() {
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-bg-main text-text-main font-sans overflow-hidden">
      {/* Header */}
      <header id="main-header" className="h-16 flex items-center justify-between px-8 bg-bg-header border-b border-border-dim flex-shrink-0">
        ...
        <div className="flex items-center gap-6">
          <div className="flex flex-col items-end">
            <span className="text-[10px] text-text-dim uppercase tracking-widest leading-none mb-1">Operator</span>
      ...
      <main className="flex-1 grid grid-cols-1 md:grid-cols-12 gap-6 p-6 overflow-hidden">
        ...
        <div className="md:col-span-4 flex flex-col gap-6 overflow-hidden">
          <section className="bg-bg-app border border-border-dim rounded-xl p-5 flex flex-col gap-4 shadow-xl">
            <div className="flex items-center justify-between">
              <h2 className="text-xs font-semibold uppercase tracking-widest text-text-dim">Active Scanner</h2>
            ...
            <AnimatePresence mode="wait">
              {step === 'scan' ? (
                ...
                >
                  ...
                  <div>
                    <p className="text-xs font-bold uppercase tracking-widest mb-1">ID Verified</p>
            ...
            <button 
              onClick={() => step === 'scan' && handleScanSuccess('MANUAL-NODE-01')}
          ...
          <section className="flex-1 bg-bg-app border border-border-dim rounded-xl p-5 overflow-hidden flex flex-col shadow-xl">
            <h2 className="text-xs font-semibold uppercase tracking-widest text-text-dim mb-4">Last Scanned</h2>
            <div className="space-y-3 overflow-y-auto custom-scrollbar flex-1">
              {history.length > 0 ? (
                history.map((item, i) => (
                  <div key={i} className="p-3 bg-bg-main border-l-2 border-brand-orange rounded flex justify-between items-center group transition-all hover:bg-bg-card">
                    <div className="flex flex-col">
                      <span className="text-xs font-mono text-brand-orange tracking-tighter truncate max-w-[120px]">{item.qrCode}</span>
                      <span className="text-[10px] text-text-dim">{item.timestamp}</span>
                    </div>
                    <span className="text-[10px] bg-brand-orange/10 text-brand-orange px-2 py-1 rounded font-bold uppercase">Logged</span>
                  </div>
                ))
              ) : (
                <div className="h-full flex items-center justify-center border border-dashed border-border-bright rounded p-8">
                  <p className="text-[10px] text-text-dim uppercase tracking-widest text-center">No active session history</p>
                </div>
              )}
            </div>
          </section>
        ...
        <div className="md:col-span-8 bg-bg-app border border-border-dim rounded-xl flex flex-col shadow-2xl overflow-hidden">
          <div className="p-6 border-b border-border-dim flex items-center justify-between bg-bg-header/50">
            <div>
              <h2 className="text-xl font-light text-white tracking-tight">Discrepancy Checklist</h2>
              <p className="text-xs text-text-dim mt-1">Select items with quantity mismatch or damage.</p>
            </div>
              ...
              {step === 'success' ? (
                ...
                >
                  ...
                  <p className="text-text-dim text-sm max-w-xs mb-8">
                    Audit log <span className="text-white font-mono">{Date.now().toString(36).toUpperCase()}</span> has been synchronized with the central repository.
                  </p>
                  <button 
                    onClick={resetApp}
                  ...
                  {step === 'scan' && (
                    <div className="absolute inset-0 bg-bg-app/80 backdrop-blur-sm z-10 flex items-center justify-center text-center p-12">
                      <div className="max-w-xs">
                        <Scan size={32} className="mx-auto mb-4 text-brand-orange animate-pulse" />
                        ...
                        <p className="text-xs text-text-dim uppercase tracking-widest leading-loose">
                          Please verify your workstation or pallet ID using the scanner in the left panel to unlock the checklist.
                        </p>
                      </div>
                    </div>
                  )}
      ...
      <footer className="h-10 px-8 bg-bg-header border-t border-border-dim flex items-center justify-between text-[10px] uppercase tracking-widest text-text-dim font-mono flex-shrink-0">
        ...
        <div className="flex gap-6">
          <span className="hidden sm:inline">WH_NODE: <span className="text-white">SE-42-DIST</span></span>
          <span>Battery: <span className="text-white">88%</span></span>
          <span className="text-[8px] opacity-40">UTC: {new Date().toISOString().replace('T', ' ').slice(0, 19)}</span>
        </div>
      </footer>
    </div>
  );
